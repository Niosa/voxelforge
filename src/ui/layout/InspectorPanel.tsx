...
const COLOR_PRESETS = [
  ...
  '#fb923c', '#facc15', '#f87171', '#a3e635', '#e2e8f0',
];
...
export function InspectorPanel() {
  ...
  useEffect(() => {
    if (!entity?.name) { setWikiData(null); return; }
    let isMounted = true;
    setLoadingWiki(true);
    ...
    fetch(`https://en.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(cleanName)}`)
      ...
      .then((data) => {
        if (isMounted && data && data.type === 'standard') {
          setWikiData({ title: data.title, extract: data.extract, thumbnail: data.thumbnail, description: data.description });
        } else if (isMounted) {
          setWikiData(null);
        }
      ...
      .catch(() => {
        if (isMounted) { setWikiData(null); setLoadingWiki(false); }
      });
  ...
  const potentialParents = Object.values(allEntities).filter(
    (e) => e.id !== selectedId && (e.type === 'continent' || e.type === 'region'),
  );
    ...
    return (
      <aside className="m-2 hidden w-80 flex-col rounded-2xl border border-white/10 bg-slate-950/85 p-4 shadow-2xl backdrop-blur-md md:flex">
        ...
        <div className="flex flex-1 flex-col items-center justify-center text-center p-4 text-slate-500">
          ...
          <p className="text-xs">Click any place or text label on the 3D globe to inspect and edit its details.</p>
        </div>
      </aside>
    );
  }
  ...
  const handleUpdateProperty = (key: string, value: string | number | boolean) => {
    ...
    if (!current) return;
    history.execute(
      new UpdateEntityCommand(entity.id, { properties: { ...current.properties, [key]: value } }),
    );
  };
    ...
    <aside className={`scrollbar-thin fixed inset-x-2 bottom-[6.9rem] z-20 max-h-[min(48dvh,calc(100dvh-11rem))] w-auto flex-col space-y-3 overflow-y-auto rounded-xl border border-white/15 bg-slate-950/95 p-3 shadow-2xl backdrop-blur-md animate-fadeIn md:static md:m-2 md:flex md:h-[calc(100dvh-5.5rem)] md:max-h-none md:w-80 md:space-y-4 md:rounded-2xl md:p-4 md:pb-12 ${activeTool === 'select' || activeTool === 'pan' ? 'flex' : 'hidden'}`}>
      ...
      <div className="flex items-center justify-between border-b border-white/10 pb-2">
        <div className="flex items-center gap-2">
          {currentFlagUrl ? (
            <img src={currentFlagUrl} alt="Flag" className="h-4 w-6 object-cover rounded border border-white/30 shadow-sm" />
          ) : (
            <span className="h-3.5 w-3.5 rounded-full border border-white/30" style={{ background: entity.color }} />
          )}
          <h2 className="text-xs font-bold uppercase tracking-wider text-slate-300">Inspector</h2>
        </div>
        <div className="flex items-center gap-2">
          <button
            type="button"
          ...
          <button
            ...
            title="Fly to Place"
          >🚀 Fly To</button>
          <button type="button" onClick={() => select(null)} className="text-xs text-slate-400 hover:text-white" aria-label="Close Inspector">✕</button>
        </div>
      </div>

      {/* Name Input */}
      <div>
        <label className="block text-[11px] font-semibold text-slate-400 uppercase tracking-wide mb-1">Place Name</label>
        <input
          type="text"
          value={entity.name}
      ...
      {/* Type Dropdown */}
      <div>
        <label className="block text-[11px] font-semibold text-slate-400 uppercase tracking-wide mb-1">Type Category</label>
        <select
          value={entity.type}
      ...
      {/* Parent Territory Picker */}
      {potentialParents.length > 0 && (
        <div>
          <label className="block text-[11px] font-semibold text-slate-400 uppercase tracking-wide mb-1">Parent Territory</label>
          <select
            value={(entity as any).parentId || ''}
            onChange={(e) => handleUpdate({ parentId: e.target.value || undefined } as any)}
            className="w-full rounded-xl border border-white/15 bg-slate-900 px-3 py-1.5 text-xs font-medium text-slate-200 focus:border-teal-400 focus:outline-none transition cursor-pointer"
          >
      ...
      {enableFlagAssignment && (
        <div className="rounded-xl border border-teal-500/30 bg-slate-900/80 p-3 space-y-3 shadow-inner">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-bold uppercase tracking-wider text-teal-300 flex items-center gap-1.5">🚩 Flag & Heraldry Banner</span>
            {currentFlagUrl && (
              <button type="button" onClick={() => handleUpdateProperty('flagUrl', '')} className="text-[10px] text-rose-400 hover:text-rose-300 font-semibold cursor-pointer">Remove Flag</button>
            )}
          </div>
          {currentFlagUrl ? (
            <div className="relative overflow-hidden rounded-xl border border-white/20 bg-slate-950 p-2 shadow-md">
              <img src={currentFlagUrl} alt={`${entity.name} Flag`} className="h-28 w-full object-cover rounded-lg shadow" />
            </div>
          ) : (
          ...
          <div className="space-y-2">
            <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-wide">Upload Flag Image</label>
            <input
              ...
              onChange={(e) => {
                ...
                if (file) {
                  ...
                  reader.onload = (ev) => { const r = ev.target?.result as string; if (r) handleUpdateProperty('flagUrl', r); };
            ...
            />
            <div className="pt-1">
              <span className="block text-[10px] font-semibold text-slate-400 uppercase tracking-wide mb-1">Or Image URL</span>
              <input
      ...
      {entity.geometry.type === 'Point' && (
        <div className="rounded-xl border border-teal-500/30 bg-slate-900/80 p-3 space-y-3 shadow-inner">
          <div className="text-[11px] font-bold uppercase tracking-wider text-teal-300 flex items-center gap-1.5">📍 Google Earth Pin Style</div>
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-wide mb-1">Pin Shape</label>
              <select value={(entity.properties.pinStyle as string) || 'teardrop'} onChange={(e) => handleUpdateProperty('pinStyle', e.target.value)} className="w-full rounded-lg border border-white/15 bg-slate-950 px-2 py-1 text-xs font-medium text-slate-200 focus:border-teal-400 focus:outline-none transition cursor-pointer">
            ...
            <div>
              <label className="block text-[10px] font-semibold text-slate-400 uppercase tracking-wide mb-1">Icon Badge</label>
              <select value={(entity.properties.pinIcon as string) || 'pin'} onChange={(e) => handleUpdateProperty('pinIcon', e.target.value)} className="w-full rounded-lg border border-white/15 bg-slate-950 px-2 py-1 text-xs font-medium text-slate-200 focus:border-teal-400 focus:outline-none transition cursor-pointer">
          ...
          <div>
            <div className="flex justify-between items-center mb-1">
              <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wide">3D Tether Altitude</label>
              <span className="text-xs font-mono text-teal-300">{((entity.properties.pinHeight as number) || 0).toLocaleString()} m</span>
            </div>
            <input type="range" min="0" max="5000" step="50" value={(entity.properties.pinHeight as number) || 0} onChange={(e) => handleUpdateProperty('pinHeight', parseInt(e.target.value, 10))} className="w-full accent-teal-400 cursor-pointer" />
          </div>
        </div>
      )}

      {/* 3D Buildings for City & Town */}
      {(entity.type === 'city' || entity.type === 'town') && (
        <div className="rounded-xl border border-teal-500/30 bg-slate-900/80 p-3 space-y-2.5 shadow-inner">
          <div className="text-[11px] font-bold uppercase tracking-wider text-teal-300 flex items-center justify-between">🏢 3D Buildings & Vehicles</div>
          <div className="flex items-center justify-between">
            <span className="text-xs font-medium text-slate-300">Auto 3D Buildings</span>
            <button type="button" onClick={() => handleUpdateProperty('generate3DBuildings', !(entity.properties.generate3DBuildings !== false))} className={`rounded-lg px-2.5 py-1 text-xs font-bold transition cursor-pointer ${entity.properties.generate3DBuildings !== false ? 'bg-teal-500/20 text-teal-200 border border-teal-500/40' : 'bg-white/5 text-slate-400 border border-white/10'}`}>
      ...
      {entity.geometry.type !== 'Point' && childPlaces.length > 0 && (
        <div className="rounded-xl border border-teal-500/30 bg-teal-950/20 p-3 space-y-2">
          <div className="text-[11px] font-bold uppercase tracking-wider text-teal-300">🏰 Cities & Fortresses in {entity.name}</div>
          <div className="space-y-1.5">
            {childPlaces.slice(0, 5).map((child) => (
              <div key={child.id} className="flex items-center justify-between rounded-lg bg-slate-900/80 px-2.5 py-1.5 text-xs text-slate-200">
                <span className="font-medium truncate max-w-[140px]">{child.name}</span>
                <button type="button" onClick={() => { select(child.id); flyToEntity(child); }} className="rounded bg-teal-500/20 px-2 py-0.5 text-[10px] font-bold text-teal-300 hover:bg-teal-500/30 transition">🚀 Fly To</button>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Biome / Topography / Climate */}
      {entity.geometry.type !== 'Point' && (
        <div className="space-y-3">
          <div>
            <label className="block text-[11px] font-semibold text-teal-300 uppercase tracking-wide mb-1">🌱 Terrain Biome Style</label>
            <select value={currentBiome} onChange={(e) => handleUpdateProperty('biome', e.target.value)} className="w-full rounded-xl border border-teal-500/30 bg-slate-900 px-3 py-1.5 text-xs font-semibold text-teal-200 focus:border-teal-400 focus:outline-none transition cursor-pointer">
          ...
          <div>
            <label className="block text-[11px] font-semibold text-slate-400 uppercase tracking-wide mb-1">🌤️ Climate Preset</label>
            <select value={(entity.properties.climate as string) || 'temperate'} onChange={(e) => handleUpdateProperty('climate', e.target.value)} className="w-full rounded-xl border border-white/15 bg-slate-900 px-3 py-1.5 text-xs font-medium text-slate-200 focus:border-teal-400 focus:outline-none transition cursor-pointer">
      ...
      {entity.geometry.type !== 'Point' && (
        <div className="flex items-center justify-between rounded-xl border border-white/10 bg-slate-900/60 p-2.5">
          ...
          <button type="button" onClick={() => handleUpdateProperty('showFill', !showFill)} className={`rounded-lg px-3 py-1 text-xs font-semibold transition ${showFill ? 'bg-teal-500/20 text-teal-200 border border-teal-500/40' : 'bg-white/5 text-slate-400 border border-white/10'}`}>
      ...
      <div>
        <label className="block text-[11px] font-semibold text-slate-400 uppercase tracking-wide mb-1">Description & Lore</label>
        <textarea rows={2} value={entity.description} onChange={(e) => handleUpdate({ description: e.target.value })} placeholder="Add notes about this territory..." className="w-full rounded-xl border border-white/15 bg-slate-900 px-3 py-1.5 text-xs text-slate-200 focus:border-teal-400 focus:outline-none transition resize-none" />
      </div>
      ...
      <div className="rounded-xl border border-teal-500/30 bg-teal-950/30 p-3 space-y-2">
        ...
        {wikiData ? (
          <div className="space-y-2 text-xs">
            {wikiData.thumbnail && <img src={wikiData.thumbnail.source} alt={wikiData.title} className="h-28 w-full rounded-lg object-cover border border-white/10" />}
            {wikiData.description && <div className="text-[10px] font-semibold text-teal-200 uppercase tracking-wider">{wikiData.description}</div>}
            <p className="text-[11px] leading-relaxed text-slate-300 line-clamp-4">{wikiData.extract}</p>
          </div>
        ) : !loadingWiki ? (
          <p className="text-[11px] text-slate-400 italic">Custom world entity. Add description above.</p>
        ) : null}
      </div>
      ...
      <div className="space-y-2">
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="block text-[11px] font-semibold text-slate-400 uppercase tracking-wide mb-1">Fill Color</label>
            <div className="flex items-center gap-2">
              <input type="color" value={entity.color} onChange={(e) => handleUpdate({ color: e.target.value })} className="h-8 w-10 rounded-lg border border-white/20 bg-transparent cursor-pointer" />
              <span className="text-xs font-mono text-slate-400">{entity.color}</span>
            </div>
          </div>
          <div>
            <label className="block text-[11px] font-semibold text-slate-400 uppercase tracking-wide mb-1">Opacity ({Math.round(entity.fillOpacity * 100)}%)</label>
            <input type="range" min="0.1" max="1.0" step="0.05" value={entity.fillOpacity} onChange={(e) => handleUpdate({ fillOpacity: parseFloat(e.target.value) })} className="w-full accent-teal-400 cursor-pointer" />
          </div>
        </div>
        {/* Quick-pick color presets */}
        <div>
          <span className="block text-[10px] font-semibold text-slate-500 uppercase tracking-wide mb-1">Quick Colors</span>
            ...
            {COLOR_PRESETS.map((c) => (
              <button
                ...
                className={`h-5 w-5 rounded-full border-2 transition cursor-pointer ${
                  entity.color === c ? 'border-white scale-110 shadow-lg' : 'border-transparent hover:scale-110'
                }`}
                style={{ background: c }}
              />
            ))}
      ...
      <div className="pt-1">
        {confirmDelete ? (
          <div className="flex gap-2">
            <button type="button" onClick={handleDelete} className="flex-1 rounded-xl border border-rose-500 bg-rose-500/20 py-2 text-xs font-bold text-rose-300 hover:bg-rose-500/30 transition">⚠️ Confirm Delete</button>
            <button type="button" onClick={() => setConfirmDelete(false)} className="rounded-xl border border-white/15 bg-white/5 px-3 py-2 text-xs text-slate-400 hover:bg-white/10 transition">Cancel</button>
          </div>
        ) : (
          <button type="button" onClick={handleDelete} className="w-full rounded-xl border border-rose-500/30 bg-rose-500/10 py-2 text-xs font-semibold text-rose-300 hover:bg-rose-500/20 transition">
            🗑️ Delete Place
          </button>
        )}
      </div>
